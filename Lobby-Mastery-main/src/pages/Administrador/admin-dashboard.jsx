  ...
  const handleToggleAccess = async (userId, sala, currentValue) => {
    ...
    if (result.success) {
      setUsers(prev =>
        prev.map(u => {
          if (u.id === userId) {
            const updated = { ...u, [`acceso_${sala}`]: !currentValue };
            const newValue = !currentValue;
            
            // Si se activa maestría, también activar máquina y códigos
            if (sala === 'maestria' && newValue === true) {
              updated.acceso_maquina = true;
              updated.acceso_codigo = true;
            }
            
            // Si se activa máquina, también activar códigos
            if (sala === 'maquina' && newValue === true) {
              updated.acceso_codigo = true;
            }
      ...
      const newValue = !currentValue;
      const statsUpdate = { [sala]: newValue ? prev[sala] + 1 : prev[sala] - 1 };
      
      // Si se activa maestría, también actualizar stats de máquina y códigos
      if (sala === 'maestria' && newValue === true) {
        setStats(prev => ({
          ...prev,
          ...statsUpdate,
          maquina: prev.maquina + 1,
          codigo: prev.codigo + 1,
        }));
      } 
      // Si se activa máquina, también actualizar stats de códigos
      else if (sala === 'maquina' && newValue === true) {
        setStats(prev => ({
          ...prev,
          ...statsUpdate,
          codigo: prev.codigo + 1,
