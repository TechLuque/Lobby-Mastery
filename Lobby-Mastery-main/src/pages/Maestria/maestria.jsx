...
const Maestria = () => {
  ...
  useEffect(() => {
    const checkAccess = async () => {
      try {
        ...
        setUserData(data);
        setHasAccess(data?.acceso_maestria === true);
      } catch (err) {
  ...
  const handleJoinZoom = async () => {
    setJoiningZoom(true);
    const meetingWindow = window.open('', 'zoom_maestria', 'width=1200,height=800');
    const result = await joinZoomMeeting('maestria', 'Maestría', meetingWindow);
  ...
  return (
    <div className="product-page sala-maestria">
      {/* NAVBAR */}
      <nav className="navbar">
        <div className="logo" onClick={() => navigate('/lobby')} style={{ cursor: 'pointer' }}><img src={logoLuque} alt="Luque Academy" className="logo-img" /></div>
        <ul className="nav-links">
      ...
      <video className="video-background" autoPlay muted loop playsInline preload="auto">
        <source src="/videos/LOGO_MST_BLANCO.mp4" type="video/mp4" />
      </video>
